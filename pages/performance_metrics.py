...
# ─── 👑 CLOUD-ONLY NETWORK SESSION INITIALIZATION ───
# Detects if running on Streamlit Cloud via standard environment flags
is_cloud = os.environ.get("STREAMLIT_RUNTIME_ENV") == "cloud" or "STREAMLIT_SHARE_LINKS" in os.environ

if "session" not in st.session_state:
    if is_cloud:
        # Apply the restrictive proxy session ONLY when deployed to the cloud
        ...
        # Explicitly set to None locally so yfinance uses native, unblocked requests
        ...
        with tab1:
            ...
            with st.spinner("Analyzing current price vs analyst target matrices..."):
                for _, row in df_stocks.iterrows():
                    ...
                    try:
                        # Pass the session if it exists (Cloud), otherwise call natively (Local)
            ...
            st.dataframe(
                df_forecast.style.format({
                    "Current Price": safe_currency, "Low Target": safe_currency, "Median Target": safe_currency,
            ...
            st.info(
                "💡 **Interpretation:** A positive green percentage indicates remaining growth runway to hit target. Negative red implies asset has currently outgrown that target limit.")
            ...
            if selected_asset:
                ...
                try:
                    ...
                    if shares_outstanding:
                        ...
                        with col_param1:
                            input_val = st.number_input("Baseline Free Cash Flow (in chosen unit)",
                                                        value=float(default_scaled_val), format="%.2f")
                        with col_param2:
                            unit_choice = st.selectbox("Unit Scale", options=["Billions", "Millions", "Absolute Value"],
                                                       index=0 if default_scale == "Billions" else 1)

                        # Convert localized layout values back up to true absolute integers
                        if unit_choice == "Billions":
                            start_fcf = input_val * 1_000_000_000
                        elif unit_choice == "Millions":
                            start_fcf = input_val * 1_000_000
                        else:
                            start_fcf = input_val

                        col_p3, col_p4, col_p5 = st.columns(3)
                        with col_p3:
                            growth_rate = st.slider("Expected Growth (Y1-5 %)", min_value=-20.0, max_value=50.0,
                        ...
                        with col_p5:
                            terminal_growth = st.slider("Perpetual Terminal Growth (%)", min_value=0.0, max_value=5.0,
                                                        value=2.5, step=0.1) / 100.0
                        ...
                        with res_col2:
                            ...
                            st.metric(label="Estimated DCF Fair Value", value=f"{c_symbol}{fair_price_per_share:.2f}",
                        ...
                        st.caption(
                            f"💡 **Terminal Value Note:** The present value of all cash flows beyond Year 5 evaluates to **{format_compact(discounted_tv)}** based on your terminal configurations.")
                    else:
                        st.error(
